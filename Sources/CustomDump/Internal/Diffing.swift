  ...
  func applying(_ difference: CollectionDifference<Element>) -> Self? {
    ...
    do {
      ...
      try difference._fastEnumeratedApply { change in
        ...
        precondition(enumeratedOriginals <= self.count)
      }
      if currentIndex < self.endIndex {
        result.append(contentsOf: self[currentIndex...])
      }
      precondition(result.count == self.count + enumeratedInserts - enumeratedRemoves)
    } catch {
      return nil
    }
    ...
    C.Element == D.Element
{
  ...
  func _formChanges(
    ...
    for d in stride(from: trace.count &- 1, to: 0, by: -1) {
      ...
      precondition((x == prev_x && y > prev_y) || (y == prev_y && x > prev_x))
      if y != prev_y {
